...
/* ---------------- Active navigation + toast feedback (makes every link/button "do" something) ---------------- */
...
const notifications = [
  { text: 'New high-risk personnel flagged in Unit Alpha', time: '5m ago' },
  { text: 'Weekly welfare report generated', time: '1h ago' },
  { text: 'ML model retraining completed', time: '3h ago' },
]
...
function handleProfileAction(action: string) {
  closeDropdowns()
  showToast(action === 'Logout' ? 'Logged out successfully' : `Opening ${action}...`)
}
...
/* ---------------- Search (filters the High Risk Personnel table live) ---------------- */
  ...
  <div class="flex min-h-screen bg-[#0b1220] text-slate-100">
    ...
    <aside class="flex w-64 shrink-0 flex-col border-r border-white/5 bg-[#0d1526]">
      <div class="flex items-center gap-3 px-5 py-5">
        <div class="flex h-8 w-8 items-center justify-center rounded-xl bg-blue-600/20 text-blue-400">
      ...
      <nav class="flex-1 space-y-1 overflow-y-auto px-3 pb-4">
        <a
          ...
          class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold shadow-sm"
          :class="activeNavItem === 'Dashboard' ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-white/5'"
        >
        ...
        <div v-for="section in navSections" :key="section.label" class="pt-1">
          ...
          >
            <span class="flex items-center gap-3">
              <svg v-if="section.icon === 'users'" class="h-3.5 w-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <svg v-else-if="section.icon === 'building'" class="h-3.5 w-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <svg v-else-if="section.icon === 'chart'" class="h-3.5 w-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 19V6l7 7-7 6zM4 5v14"></path>
              </svg>
              <svg v-else-if="section.icon === 'shield'" class="h-3.5 w-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <svg v-else class="h-3.5 w-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div v-show="openSections[section.label]" class="ml-6 mt-1 space-y-0.5 border-l border-white/5 pl-4">
            <a
              ...
              :class="activeNavItem === item ? 'bg-white/5 text-white' : 'text-slate-400 hover:bg-white/5 hover:text-slate-200'"
            >
              {{ item }}
            </a>
          </div>
    ...
    <div class="flex min-h-screen flex-1 flex-col">
      ...
      <header class="flex items-center gap-4 border-b border-white/5 bg-[#0d1526] px-6 py-3.5">
        <div class="relative max-w-md flex-1">
          <svg class="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="ml-auto flex items-center gap-4">
          <div class="relative" data-dropdown-root>
            ...
            >
              ...
              <span v-if="notifications.length" class="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-red-500"></span>
            </button>

            <div
              v-if="showNotifications"
              class="absolute right-0 z-20 mt-2 w-72 rounded-xl border border-white/10 bg-[#0d1526] p-2 shadow-xl"
            >
              <p class="px-2 py-1.5 text-xs font-semibold text-slate-300">Notifications</p>
              <div
          ...
          <div class="relative border-l border-white/10 pl-4" data-dropdown-root>
            <button type="button" @click.stop="toggleProfileMenu" class="flex items-center gap-2.5">
              ...
              <svg class="h-3.5 w-3.5 text-slate-500 transition-transform" :class="{ 'rotate-180': showProfileMenu }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
              </svg>
            </button>
            ...
            <div
              ...
              class="absolute right-0 z-20 mt-2 w-44 rounded-xl border border-white/10 bg-[#0d1526] p-1.5 shadow-xl"
            >
              <button
                v-for="action in ['Profile', 'Account Settings', 'Logout']"
                :key="action"
                type="button"
      ...
      <main class="flex-1 space-y-6 p-6">
        ...
        <div v-else-if="error" class="flex flex-col items-center gap-3 rounded-2xl border border-red-500/20 bg-red-500/5 p-16 text-center">
          ...
          <button type="button" @click="retryFetch" class="mt-2 rounded-lg bg-blue-600 px-4 py-2 text-xs font-semibold text-white hover:bg-blue-500">Retry</button>
        </div>
          ...
          <div class="grid grid-cols-1 gap-4 xl:grid-cols-[1.6fr_1.2fr_1.2fr]">
            ...
            <div class="rounded-2xl border border-white/5 bg-[#0d1526] p-5">
              ...
              <table class="mt-4 w-full text-left text-xs">
                ...
                <tbody>
                  <tr v-if="!filteredHighRiskPersonnel.length">
                    <td colspan="5" class="py-4 text-center text-slate-500">
                      {{ searchQuery ? 'No personnel match your search.' : 'No high risk personnel.' }}
                    </td>
                  </tr>
                  <tr v-for="p in filteredHighRiskPersonnel" :key="p.id" class="border-t border-white/5">
    ...
    <div
      v-if="toastMessage"
      class="fixed bottom-6 right-6 z-50 rounded-lg border border-white/10 bg-[#0d1526] px-4 py-2.5 text-xs font-medium text-slate-200 shadow-xl"
    >
      {{ toastMessage }}
    </div>
  </div>
