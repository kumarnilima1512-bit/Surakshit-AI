...
/* ======================================================================
   API CONTRACT
   ----------------------------------------------------------------------
   GET  /api/commander/alerts               -> AlertsPageData
   POST /api/commander/alerts/:id/acknowledge -> { ok: true }
   ====================================================================== */
...
/* ---------------- Data fetching ---------------- */
const { data, pending: loading, error, refresh: fetchAlerts } =
...
const severityTone: Record<Severity, { badge: string; iconBg: string }> = {
  Critical: { badge: 'bg-red-500/15 text-red-400', iconBg: 'bg-red-500/15 text-red-400' },
  Warning: { badge: 'bg-amber-500/15 text-amber-400', iconBg: 'bg-amber-500/15 text-amber-400' },
  Info: { badge: 'bg-blue-500/15 text-blue-400', iconBg: 'bg-blue-500/15 text-blue-400' },
}
...
const filteredAlerts = computed<AlertRecord[]>(() => {
  ...
  return list.filter((a) => {
    const matchesQuery = !q || a.title.toLowerCase().includes(q) || a.detail.toLowerCase().includes(q) || a.subUnit.toLowerCase().includes(q)
    const matchesSeverity = severityFilter.value === 'All' || a.severity === severityFilter.value
    const matchesAck = showAcknowledged.value || !a.acknowledged
    return matchesQuery && matchesSeverity && matchesAck
  })
})

/* ---------------- Acknowledge (real action) ---------------- */
const acknowledgingId = ref<string | null>(null)
async function acknowledgeAlert(id: string) {
  if (!data.value) return
  acknowledgingId.value = id
  try {
    await $fetch(`/api/commander/alerts/${id}/acknowledge`, { method: 'POST' })
    const target = data.value.alerts.find((a) => a.id === id)
    if (target) target.acknowledged = true
  } catch {
    // Leave state unchanged; button re-enables so the commander can retry.
  } finally {
...
/>
    ...
    <div class="flex min-h-screen flex-1 flex-col">
      ...
      <main class="flex-1 space-y-4 p-6">
        <div>
          <h1 class="text-xl font-extrabold tracking-tight text-white">Risk Alerts</h1>
          <p class="mt-0.5 text-xs text-slate-400">Automated alerts flagged across your unit</p>
        </div>
        ...
        <div v-else-if="error" class="flex flex-col items-center gap-3 rounded-2xl border border-red-500/20 bg-red-500/5 p-16 text-center">
          ...
          <p class="text-sm font-semibold text-red-300">Couldn't load alerts</p>
          <p class="text-xs text-slate-400">{{ error.message }} — expected data from <code class="rounded bg-white/5 px-1.5 py-0.5">/api/commander/alerts</code></p>
          <button type="button" @click="fetchAlerts()" class="mt-2 rounded-lg bg-emerald-600 px-4 py-2 text-xs font-semibold text-white hover:bg-emerald-500">Retry</button>
        </div>
        ...
        <div v-else-if="data" class="rounded-2xl border border-white/5 bg-[#0d1526] p-5">
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="relative">
              ...
              <input v-model="search" type="text" placeholder="Search alerts..." class="w-56 rounded-lg border border-white/10 bg-white/5 py-1.5 pl-8 pr-3 text-xs text-slate-200 placeholder-slate-500 outline-none focus:border-emerald-500/50">
            </div>
            <div class="flex flex-wrap items-center gap-3">
              <div class="flex items-center gap-1.5">
                <Filter :size="12" class="mr-1 text-slate-500" />
                <button v-for="opt in severityOptions" :key="opt" type="button" @click="severityFilter = opt" class="rounded-md px-2.5 py-1 text-[11px] font-semibold" :class="severityFilter === opt ? 'bg-emerald-600 text-white' : 'bg-white/5 text-slate-400 hover:bg-white/10'">{{ opt }}</button>
              </div>
              <label class="flex items-center gap-1.5 text-[11px] text-slate-400">
                <input v-model="showAcknowledged" type="checkbox" class="rounded border-white/20 bg-white/5 text-emerald-500 focus:ring-emerald-500/50">
                Show acknowledged
              </label>
            </div>
          </div>
          ...
          <div class="mt-4 space-y-2">
            <div v-if="!filteredAlerts.length" class="py-8 text-center text-xs text-slate-500">
              {{ search || severityFilter !== 'All' ? 'No alerts match your filters.' : 'No alerts right now.' }}
            </div>
            <div
              v-for="a in filteredAlerts" :key="a.id"
              class="flex items-start gap-3 rounded-xl border border-white/5 p-3.5"
              :class="a.acknowledged ? 'opacity-60' : ''"
            >
              <div class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg" :class="severityTone[a.severity].iconBg">
                <AlertTriangle :size="15" :stroke-width="1.5" />
              </div>
              <div class="min-w-0 flex-1">
                <div class="flex flex-wrap items-center gap-2">
                  <p class="text-xs font-semibold text-slate-200">{{ a.title }}</p>
                  <span class="rounded-md px-2 py-0.5 text-[10px] font-bold" :class="severityTone[a.severity].badge">{{ a.severity }}</span>
                </div>
                <p class="mt-0.5 text-[11px] text-slate-400">{{ a.detail }}</p>
                <p class="mt-1 text-[10px] text-slate-500">{{ a.subUnit }} &middot; {{ a.time }}</p>
              </div>
              <div class="flex shrink-0 items-center gap-3">
                <NuxtLink v-if="a.personnelId" :to="`/commander/personnel/${a.personnelId}`" class="text-[11px] font-semibold text-emerald-400 hover:underline">Profile</NuxtLink>
                <button
                  v-if="!a.acknowledged"
                  type="button" :disabled="acknowledgingId === a.id" @click="acknowledgeAlert(a.id)"
                  class="flex items-center gap-1 rounded-lg border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] font-semibold text-slate-200 hover:bg-white/10 disabled:opacity-50"
                >
                  <Check :size="11" />
                  {{ acknowledgingId === a.id ? 'Saving…' : 'Acknowledge' }}
                </button>
                <span v-else class="text-[11px] font-medium text-slate-500">Acknowledged</span>
              </div>
            </div>
          </div>
        </div>
