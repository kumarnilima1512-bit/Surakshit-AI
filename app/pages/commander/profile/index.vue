<!--
  pages/commander/profile/index.vue
  URL: /commander/profile  (sidebar label: "My Profile")
...
/* ======================================================================
   API CONTRACT
   ----------------------------------------------------------------------
   GET  /api/commander/profile -> ProfileData
   PUT  /api/commander/profile { name, phone, email } -> ProfileData
   ====================================================================== */
...
async function saveProfile() {
  ...
  saveError.value = null
  try {
    const updated = await $fetch<ProfileData>('/api/commander/profile', {
      method: 'PUT',
    ...
    <div class="flex min-h-screen flex-1 flex-col">
          ...
          <div class="relative border-l border-white/10 pl-4" data-dropdown-root>
            <button type="button" @click.stop="toggleProfile" class="flex items-center gap-2.5">
              ...
              <div class="text-left leading-tight">
                <p class="text-sm font-semibold text-white">{{ data?.rank }} {{ data?.name }}</p>
      ...
      <main class="flex-1 space-y-4 p-6">
        <div>
          ...
          <p class="mt-0.5 text-xs text-slate-400">Your command and contact details</p>
        </div>
        ...
        <div v-else-if="error" class="flex flex-col items-center gap-3 rounded-2xl border border-red-500/20 bg-red-500/5 p-16 text-center">
          ...
          <p class="text-xs text-slate-400">{{ error.message }} — expected data from <code class="rounded bg-white/5 px-1.5 py-0.5">/api/commander/profile</code></p>
        ...
        <div v-else-if="data" class="max-w-2xl rounded-2xl border border-white/5 bg-[#0d1526] p-6">
          <div class="flex items-center gap-4">
            ...
            <div>
              <p class="text-lg font-bold text-white">{{ data.rank }} {{ data.name }}</p>
              <p class="text-xs text-slate-400">{{ data.unitName }} &middot; {{ data.unitCode }}</p>
