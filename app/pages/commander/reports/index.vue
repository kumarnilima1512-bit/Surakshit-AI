...
/* ======================================================================
   API CONTRACT
   ----------------------------------------------------------------------
   GET  /api/commander/reports            -> ReportsData
   POST /api/commander/reports/generate    { type, from, to } -> ReportRecord
   ====================================================================== */

type ReportType = 'Unit Risk Summary' | 'Personnel Readiness Report' | 'Risk Alert Log' | 'Monthly Command Report'
...
const { data, pending: loading, error, refresh: fetchReports } =
  await useFetch<ReportsData>('/api/commander/reports')
...
const reportTypes: ReportType[] = ['Unit Risk Summary', 'Personnel Readiness Report', 'Risk Alert Log', 'Monthly Command Report']
...
async function generateReport() {
  ...
  try {
    const created = await $fetch<ReportRecord>('/api/commander/reports/generate', {
      method: 'POST',
...
const typeBadge: Record<ReportType, string> = {
  ...
  'Personnel Readiness Report': 'bg-emerald-500/15 text-emerald-400',
  'Risk Alert Log': 'bg-red-500/15 text-red-400',
  'Monthly Command Report': 'bg-violet-500/15 text-violet-400',
}
  ...
  <div class="flex min-h-screen bg-[#0b1220] text-slate-100">
    ...
    <div class="flex min-h-screen flex-1 flex-col">
      ...
      <main class="flex-1 space-y-4 p-6">
        <div>
          <h1 class="text-xl font-extrabold tracking-tight text-white">Unit Reports</h1>
          <p class="mt-0.5 text-xs text-slate-400">Generate and download reports for your unit</p>
        </div>
        ...
        <div v-else-if="error" class="flex flex-col items-center gap-3 rounded-2xl border border-red-500/20 bg-red-500/5 p-16 text-center">
          ...
          <p class="text-xs text-slate-400">{{ error.message }} — expected data from <code class="rounded bg-white/5 px-1.5 py-0.5">/api/commander/reports</code></p>
