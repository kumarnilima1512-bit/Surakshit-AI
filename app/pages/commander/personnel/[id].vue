...
/* ======================================================================
   API CONTRACT
   ----------------------------------------------------------------------
   GET  /api/commander/personnel/:id           -> PersonnelDetail
   POST /api/commander/personnel/:id/follow-up  { date: string } -> { ok: true }
   ====================================================================== */
...
const { data, pending: loading, error, refresh: fetchPersonnel } =
  await useFetch<PersonnelDetail>(() => `/api/commander/personnel/${personnelId.value}`)

/* ---------------- Risk tone tokens ---------------- */
const riskTone: Record<RiskLevel, { text: string; ring: string; badgeBg: string }> = {
  Low: { text: 'text-emerald-400', ring: '#34d399', badgeBg: 'bg-emerald-500/15' },
  Moderate: { text: 'text-blue-400', ring: '#60a5fa', badgeBg: 'bg-blue-500/15' },
  Elevated: { text: 'text-amber-400', ring: '#fbbf24', badgeBg: 'bg-amber-500/15' },
  High: { text: 'text-red-400', ring: '#f87171', badgeBg: 'bg-red-500/15' },
}
...
function trendPath(points: StressPoint[]) {
  ...
  return points.map((p, i) => `${i === 0 ? 'M' : 'L'} ${i * stepX} ${chartH - (p.score / chartMax) * chartH}`).join(' ')
}
...
async function scheduleFollowUp() {
  ...
  try {
    await $fetch(`/api/commander/personnel/${personnelId.value}/follow-up`, {
      method: 'POST',
    ...
    scheduleError.value = 'Could not schedule the follow-up. Please try again.'
  } finally {
...
<template>
  <div class="min-h-screen bg-[#0b1220] p-6 text-slate-100">
    <NuxtLink to="/commander/personnel" class="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-400 hover:text-slate-200">
      <ArrowLeft :size="14" />
      Back to My Personnel
    </NuxtLink>
    ...
    <div v-else-if="error" class="mt-4 flex flex-col items-center gap-3 rounded-2xl border border-red-500/20 bg-red-500/5 p-16 text-center">
      ...
      <p class="text-xs text-slate-400">{{ error.message }} — expected data from <code class="rounded bg-white/5 px-1.5 py-0.5">/api/commander/personnel/{{ personnelId }}</code></p>
        ...
        <div class="rounded-2xl border border-white/5 bg-[#0d1526] p-5 xl:col-span-2">
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-xs text-slate-400">{{ data.rank }} &middot; {{ data.subUnit }}</p>
        ...
        <div class="rounded-2xl border border-white/5 bg-[#0d1526] p-5">
          ...
          <div class="relative mx-auto mt-3 flex h-28 w-28 items-center justify-center">
            <svg viewBox="0 0 160 160" class="h-28 w-28 -rotate-90">
              ...
              <circle cx="80" cy="80" r="68" fill="none" :stroke="riskTone[data.riskLevel].ring" stroke-width="12" stroke-linecap="round" :stroke-dasharray="gaugeDashArray(data.currentStressScore, data.maxStressScore)" />
            </svg>
      ...
      <div class="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-[1.8fr_1fr]">
        <div class="rounded-2xl border border-white/5 bg-[#0d1526] p-5">
          ...
          <svg v-if="data.stressTrend.length" :viewBox="`0 0 ${chartW} ${chartH + 26}`" class="mt-4 h-48 w-full">
            <defs>
              <linearGradient id="cmdDetailTrendFill" x1="0" y1="0" x2="0" y2="1">
            ...
            <path :d="`${trendPath(data.stressTrend)} L ${chartW} ${chartH} L 0 ${chartH} Z`" fill="url(#cmdDetailTrendFill)" />
            ...
            <table class="w-full min-w-[360px] text-left text-xs">
              ...
              <tbody>
                ...
                <tr v-for="(row, i) in data.recentAssessments" :key="i" class="border-t border-white/5">
                  ...
                  <td class="py-2.5 text-right font-semibold" :class="riskTone[row.riskLevel].text">{{ row.riskLevel }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="rounded-2xl border border-white/5 bg-[#0d1526] p-5">
          ...
          <input v-model="followUpDate" type="date" class="mt-3 w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs text-slate-200 outline-none focus:border-emerald-500/50">
          <button type="button" :disabled="!followUpDate || scheduling" @click="scheduleFollowUp" class="mt-2 w-full rounded-lg bg-blue-600 py-2 text-xs font-semibold text-white hover:bg-blue-500 disabled:opacity-50">
            {{ scheduling ? 'Scheduling…' : 'Schedule' }}
          </button>
          <p v-if="scheduleError" class="mt-2 text-[11px] text-red-400">{{ scheduleError }}</p>
        </div>
      </div>
    </template>
  </div>
