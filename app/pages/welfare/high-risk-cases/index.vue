  ...
  <div class="flex min-h-screen bg-[#0b1220] text-slate-100">
    ...
    <div class="flex min-h-screen flex-1 flex-col md:ml-0">
      ...
      <main class="flex-1 space-y-4 p-3 sm:p-6">
        <div>
          <h1 class="text-lg font-extrabold tracking-tight text-white sm:text-xl">High-Risk Cases</h1>
        ...
        <template v-else-if="data">
          ...
          <div class="grid grid-cols-2 gap-3 sm:gap-4 sm:grid-cols-3 lg:grid-cols-5">
            <div v-for="s in data.summary" :key="s.label" class="rounded-2xl border border-white/5 bg-[#0d1526] p-3 sm:p-4">
              ...
              <p class="mt-1 text-xl font-extrabold text-white sm:text-2xl">{{ s.value }}</p>
            </div>
          </div>

          <!-- Table -->
          <div class="rounded-2xl border border-white/5 bg-[#0d1526] p-3 sm:p-5">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div class="relative w-full sm:w-auto">
                <Search :size="14" class="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-500" />
                <input v-model="search" type="text" placeholder="Search cases..." class="w-full rounded-lg border border-white/10 bg-white/5 py-1.5 pl-8 pr-3 text-xs text-slate-200 placeholder-slate-500 outline-none focus:border-emerald-500/50 sm:w-56">
              </div>
            ...
            <div class="mt-4 -mx-3 overflow-x-auto px-3 sm:mx-0 sm:px-0">
