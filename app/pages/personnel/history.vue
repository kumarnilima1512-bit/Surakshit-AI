...
const highestStress = computed(() => {
  ...
  return Math.max(
    ...assessments.value.map(
      (assessment) => assessment.stressScore,
    ),
  )
})
...
<template>
  <div class="flex min-h-screen bg-[#0b1220] text-slate-100">
    ...
    <!-- Sidebar -->
    <aside
      class="fixed inset-y-0 left-0 z-50 flex w-64 shrink-0 flex-col border-r border-white/5 bg-[#0d1526] transition-transform duration-200 lg:static lg:translate-x-0"
      :class="
        mobileMenuOpen
          ? 'translate-x-0'
          : '-translate-x-full'
    ...
    <div class="flex min-h-screen min-w-0 flex-1 flex-col">
      ...
      <!-- Header -->
      <header
        class="flex min-h-[65px] items-center gap-3 border-b border-white/5 bg-[#0d1526] px-4 py-3 sm:px-6"
      >
        <button
          type="button"
        ...
        <div class="ml-auto flex shrink-0 items-center gap-1 sm:gap-4">
          ...
          <NuxtLink
            to="/personnel/profile"
            class="flex items-center gap-2 border-l border-white/10 pl-2 sm:gap-2.5 sm:pl-4"
          >
            <div
              class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-700"
            >
              <User
                :size="18"
                :stroke-width="1.5"
      ...
      <main class="flex-1 space-y-5 p-4 sm:space-y-6 sm:p-6">

        <!-- Back Button -->
        <NuxtLink
          to="/personnel/dashboard"
          class="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-slate-300 transition-colors hover:bg-white/10 hover:text-white"
        >
          <ArrowLeft
            :size="15"
            :stroke-width="1.7"
        ...
        <template v-else>
          ...
          >
            ...
            >
              ...
              <p
                class="mt-4 text-2xl font-extrabold text-white sm:text-3xl"
              >
                {{ assessments.length }}
              </p>
            </div>
            ...
            >
              <div class="flex items-center gap-2">
                <div
                  class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-amber-500/15 text-amber-400"
                >
                  <TrendingUp
                    :size="18"
                    :stroke-width="1.5"
            ...
            >
              <div class="flex items-center gap-2">
                <div
                  class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-emerald-500/15 text-emerald-400"
                >
                  <TrendingUp
                    :size="18"
                    :stroke-width="1.5"
              ...
              <p
                class="mt-4 text-xl font-extrabold text-white sm:text-2xl"
              >
                {{ lowestStress ?? '—' }}
          ...
          <div
            ...
            class="rounded-2xl border border-white/5 bg-[#0d1526] p-10 text-center sm:p-16"
          >
            <div
          ...
          >
            <div
              class="flex flex-col gap-3 border-b border-white/5 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5"
            >
              <div>
              ...
              <NuxtLink
                to="/personnel/assessment/new"
                class="inline-flex w-fit rounded-lg bg-emerald-600 px-3 py-2 text-[11px] font-semibold text-white hover:bg-emerald-500"
              >
                New Assessment
              </NuxtLink>
            </div>
            ...
            <div class="overflow-x-auto">
              <table
                class="w-full min-w-[620px] text-left text-xs"
              >
                <thead>
                  <tr
                  ...
                  >
                    ...
                    <th
                      class="px-4 py-3 text-right font-medium sm:px-5"
                    >
                      Action
                    </th>
                  </tr>
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td
                      class="px-4 py-4 text-right sm:px-5"
                    >
                      <NuxtLink
                        :to="`/personnel/history/${assessment.id}`"
                        class="inline-flex items-center gap-1 whitespace-nowrap font-semibold text-blue-400 hover:text-blue-300 hover:underline"
                      >
                        View Details
            ...
            <!-- Mobile table hint -->
            <p
              class="border-t border-white/5 px-4 py-2 text-[10px] text-slate-600 sm:hidden"
            >
              Swipe horizontally to view the complete assessment history.
            </p>
          </div>
        </template>
      </main>
      ...
      <footer
        class="flex flex-col gap-2 border-t border-white/5 px-4 py-4 text-[10px] text-slate-500 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:text-[11px]"
      >
        <span>
