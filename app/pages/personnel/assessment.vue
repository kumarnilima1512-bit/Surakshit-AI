...
import EmotionCamera from '~/components/personnel/EmotionCamera.vue'

const loading = ref(false)
const errorMessage = ref('')

const facialEmotion = ref('')
const facialEmotionConfidence = ref(0)
...
function handleEmotionDetected(result: {
  emotion: string
  confidence: number
}) {
  facialEmotion.value = result.emotion
  facialEmotionConfidence.value = result.confidence
}
...
async function predictStress() {
  ...
  if (!facialEmotion.value) {
    loading.value = false
    errorMessage.value =
      'Please complete the facial emotion check before submitting the assessment.'
    return
  }
  ...
  try {
    const response = await $fetch<{
      ...
      body: {
        ...form,
        emotion: facialEmotion.value,
        emotionConfidence: facialEmotionConfidence.value
      }
    })
    ...
    await navigateTo({
      ...
      query: {
        ...
        emotion: facialEmotion.value,
        emotionConfidence: facialEmotionConfidence.value.toString()
      }
    })
  } catch (error) {
...
<template>
  <div class="min-h-screen bg-slate-50 px-4 py-6 md:px-8 md:py-10">
    <div class="mx-auto max-w-5xl">
      ...
      <div class="mb-8 rounded-2xl bg-white p-5 shadow-sm md:p-8">
        <h2 class="text-lg font-semibold text-slate-900">
          Facial Emotion Check
        </h2>

        <p class="mt-1 text-sm text-slate-500">
          Allow camera access to detect your current visible facial expression.
        </p>

        <div class="mt-6">
          <EmotionCamera
            @detected="handleEmotionDetected"
          />
        </div>
        ...
        <div
          v-if="facialEmotion"
          class="mt-5 rounded-xl border border-green-200 bg-green-50 px-4 py-3"
        >
          <div class="flex items-center justify-between gap-4">
            <div>
              ...
              <p class="mt-1 text-lg font-semibold capitalize text-green-900">
                {{ facialEmotion }}
              </p>
            </div>
            ...
            <div class="text-right">
              ...
              <p class="mt-1 text-lg font-semibold text-green-900">
                {{ Math.round(facialEmotionConfidence * 100) }}%
              </p>
            </div>
          </div>
        </div>
