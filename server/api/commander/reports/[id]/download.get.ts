...
function createPersonnelReadinessReport(
  ...
  doc.text(`Total personnel assessed: ${total}`)
  ...
  doc
    ...
    .fontSize(11)
    .fillColor('#111827')
    .text('Personnel Readiness Details')
...
function createRiskAlertLog(
  ...
  doc
    ...
    .fontSize(8)
    .fillColor('#6b7280')
    .text(`Total elevated/high-risk cases: ${alertRows.length}`)
...
function createPdf(
  ...
  return new Promise((resolve, reject) => {
    ...
    } else if (report.type === 'Personnel Readiness Report') {
      createPersonnelReadinessReport(doc, rows)
    } else if (report.type === 'Risk Alert Log') {
      createRiskAlertLog(doc, rows)
    } else if (report.type === 'Monthly Command Report') {
...
export default defineEventHandler(async (event) => {
  ...
  // Get latest assessment for each personnel
